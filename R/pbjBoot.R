  ...
  if(!voxelwise){
    if(method=='robust'){#is.list(sqrtSigma)){ sqrtSigma should be a list here
      if( length(dim(boot))==0 ){ # dimension of bootstrap must be a vector of length n
        ...
        statimg = t(simplify2array( lapply(1:V, function(ind) crossprod(matrix(BsqrtInv[,ind], nrow=df, ncol=df), crossprod(sqrtSigma$X1res, sqrtSigma$res[,ind]) ) ), higher=FALSE ) )
      } else {
