...
This is the hardest part; passing the necessary files to computeStats.R produces a statMap object which retains all of the objects we need to perform SEI and visualization.
...
With these data we are now ready to pass our arguments to the `computeStats` function.
...
As a first pass, let's do the best thing and use variance images inverse weights in the regression.
...
Using voxel level weights can take a little while because all the computation must be performed separately at each voxel.
...
Because we specified the output directory nifti images of the results are stored in `outdir` and `statmap` includes only character vectors and formulas.
...
### <a id="technical_details_return"></a>Computing the statistical map with study-wise weights
...
However, the sPBJ procedure is still valid because it uses robust standard errors so any weights can be used and the standard errors are still unbiased as long there is a large number of studies used in the analysis.
Of course, the best study level weight is one that is a good estimate of the variance for the statistical map for that study. See [below](#technical_details) for technical details.


Let's run this analysis again, instead on scaled versions of the statistical maps, but using weights are that proportional to the sample size for each study.
Because we need to pass statistical maps, we will compute these maps in `R` and pass `niftiImage` objects as arguments to `computeStats`.
...
For this example we use 500 samples, but for a publication a few thousand is more appropriate.
...
where $\sigma^2_{yi}$ is the estimated variance for the subjects $y$ from study $i$, $\beta$ is the unknown parameter, and $X_{ri}$ is the vector for the covariate of interest (pain scale for study $i$).
...
$$
and then weighting by $n_i$ since
$$
\text{Var}(n_i^{-1/2} Z_i) = n_i^{-1}.
$$
[Back](#technical_details_return) to "Computing the statistical map ..."
