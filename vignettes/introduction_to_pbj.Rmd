...
The sPBJ SEI procedure is appropriate for any type of group level analysis and we demonstrated that it is robust at cluster forming thresholds (CFTs) where GRF-based methods fail.
...
### Brief aside
As an aside, it's worth noting that Gaussian random field (GRF) methods have pretty consistently been shown to fail to control the family-wise error rate at these thresholds.
This is because GRF methods assume the data are only spatially correlated, which is wholly inaccurate, due in part to interhemispheric correlation and network connectivity.
Because of this unrealistic assumption, GRF-based tools are unlikely to control the FWER across a range of CFTs and smoothing parameters.
You don't have to use sPBJ, just don't use GRF methods (Use permutation e.g. FSL's `randomise`).
...
* SEI - spatial extent inference. Using threshold contiguous cluster extents to performing inference on a statistical map.
